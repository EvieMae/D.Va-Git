...
function openRewordModal(hash) {
  ...
  const orig = body ? `${subject}\n\n${body}` : subject;
  const isHead = state.commits[0] && state.commits[0].hash === hash;
  modal({
    title: `REWORD ${hash.slice(0,7)}`,
    body: `
      <p style="font-size: 11px; color: var(--text-3);">
        ${isHead
          ? 'Rewrites the latest commit message (amend).'
          : 'Rewrites an older commit via rebase — this changes the SHA of this and every commit after it. Working tree must be clean.'}
      </p>
      <label>NEW MESSAGE</label>
      <textarea id="modal-reword-msg" style="min-height: 120px;">${escapeHtml(orig)}</textarea>
    `,
    okText: 'REWORD',
    onOk: async () => {
      const message = $('#modal-reword-msg').value.trim();
      if (!message) { toast('Message required', 'warn'); return false; }
      setStatus('Rewording…', 'busy');
      const r = await window.api.rewordCommit({ hash, message });
