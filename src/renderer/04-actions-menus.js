...
$('#amend-check').onchange = () => {
  // ticking Amend loads the last commit's message so it can be edited in place
  if ($('#amend-check').checked) {
    const head = state.commits && state.commits[0];
    const msgEl = $('#commit-msg');
    const descEl = $('#commit-desc');
    if (head && msgEl && descEl && !msgEl.value.trim() && !descEl.value.trim()) {
      msgEl.value = head.message || '';
      descEl.value = (head.body || '').trim();
    }
  }
  renderChanges();
};
