...
// Reword ANY commit. HEAD -> amend; older commits -> a non-interactive
// `rebase -i` driven by a prebuilt todo (target line = reword) plus a
// GIT_EDITOR that injects the new message. Same copy/cat trick as the
// interactive-rebase driver, so it's robust on Windows + POSIX.
ipcMain.handle('git:rewordCommit', async (event, { hash, message } = {}) => {
  try {
    if (!S.currentRepoPath) throw new Error('No repo');
    const g = core.requireRepo();
    ...
    const full = (await g.revparse([hash])).trim();
    const head = (await g.revparse(['HEAD'])).trim();
    if (full === head) {
      await g.commit(msg, undefined, ['--amend']);
      return { ok: true };
    }

    // base = parent of target, or whole history if it's the root commit
    let isRoot = false;
    try { await g.revparse([full + '^']); } catch { isRoot = true; }
    const range = isRoot ? 'HEAD' : `${full}^..HEAD`;

    // commits oldest -> newest in the range; mark the target as `reword`
    const raw = await g.raw(['log', '--reverse', '--format=%H %s', range]);
    const lines = raw.split('\n').map(l => l.trim()).filter(Boolean);
    const todo = lines.map(l => {
      const sha = l.slice(0, 40);
      const subj = l.slice(41);
      return `${sha === full ? 'reword' : 'pick'} ${sha} ${subj}`;
    }).join('\n') + '\n';

    const isWin = process.platform === 'win32';
    const ud = app.getPath('userData');
    const stamp = Date.now();
    const todoPath = path.join(ud, `dva-rw-todo-${stamp}.txt`);
    const msgPath = path.join(ud, `dva-rw-msg-${stamp}.txt`);
    const seqPath = path.join(ud, `dva-rw-seq-${stamp}` + (isWin ? '.bat' : '.sh'));
    const edPath = path.join(ud, `dva-rw-ed-${stamp}` + (isWin ? '.bat' : '.sh'));
    fs.writeFileSync(todoPath, todo, 'utf8');
    fs.writeFileSync(msgPath, msg, 'utf8');
    fs.writeFileSync(seqPath, isWin
      ? `@echo off\r\ncopy /Y "${todoPath}" %1 >NUL\r\nexit /B 0\r\n`
      : `#!/bin/sh\ncat "${todoPath}" > "$1"\n`, 'utf8');
    fs.writeFileSync(edPath, isWin
      ? `@echo off\r\ncopy /Y "${msgPath}" %1 >NUL\r\nexit /B 0\r\n`
      : `#!/bin/sh\ncat "${msgPath}" > "$1"\n`, 'utf8');
    if (!isWin) { fs.chmodSync(seqPath, 0o755); fs.chmodSync(edPath, 0o755); }

    const upstream = isRoot ? '--root' : `${full}^`;
    return await new Promise((resolve) => {
      const env = {
        ...process.env,
        GIT_SEQUENCE_EDITOR: isWin ? `"${seqPath}"` : seqPath,
        GIT_EDITOR: isWin ? `"${edPath}"` : edPath,
      };
      const ps = spawn('git', ['rebase', '-i', upstream], { cwd: S.currentRepoPath, env, shell: isWin });
      let err = '', out = '';
      ps.stdout.on('data', (c) => { out += c.toString(); });
      ...
      ps.on('error', (e) => resolve({ ok: false, error: e.message }));
      ps.on('close', (code) => {
        [todoPath, msgPath, seqPath, edPath].forEach(p => { try { fs.unlinkSync(p); } catch {} });
        if (code === 0) return resolve({ ok: true });
        // rebase stopped (dirty tree / conflict) — don't leave the repo mid-rebase
        try { spawn('git', ['rebase', '--abort'], { cwd: S.currentRepoPath, shell: isWin }); } catch {}
        const m = err || out || `git rebase exit ${code}`;
        resolve({ ok: false, error: core.friendlyGitError ? core.friendlyGitError(m) : m });
      });
    });
  } catch (e) { return { ok: false, error: e.message }; }
});
