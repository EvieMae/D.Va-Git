...
ipcMain.handle('git:deleteRemoteBranch', async (event, { remote, branch } = {}) => {
  try {
    const g = core.requireRepo();
    if (!remote || !branch) throw new Error('Remote and branch are required');
    await g.raw(['push', remote, '--delete', branch]);
    return { ok: true };
  } catch (e) {
    return { ok: false, error: core.friendlyGitError ? core.friendlyGitError(e.message) : e.message };
  }
});

ipcMain.handle('git:merge', async (event, arg) => {
  try {
    const g = core.requireRepo();
    // backward compatible: arg may be a plain ref string or an options object
    const o = (arg && typeof arg === 'object') ? arg : { ref: arg };
    const ref = o.ref;
    if (!ref) throw new Error('No branch specified to merge');
    const flags = [];
    if (o.squash) flags.push('--squash');       // stage result, don't commit
    if (o.noCommit) flags.push('--no-commit');  // merge but stop before commit
    if (o.ffOnly) flags.push('--ff-only');      // refuse if a merge commit is needed
    await g.merge([...flags, ref]);
    return { ok: true, squash: !!o.squash };
  } catch (e) { return { ok: false, error: e.message }; }
});
